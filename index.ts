...
const testParameter = (name: string, filters: RegExp[]) =>
...
const normalizeDataURL = (urlString: string, { stripHash }: Options) => {
  ...
  let { hash, type, data } = match.groups as MatchGroups;
...
```
*/
export const normalizeUrl = (urlString: string, options?: Options): string => {
  options = {
